(function () {
  ...
  /** @constant {string} Created By DCHAO - 16/12/2025 */
  const LS_KEY = "misa_candidates";
  /** @constant {number} Created By DCHAO - 16/12/2025 */
  const DEFAULT_PAGE_SIZE = 25;

  /**
   * Trạng thái UI cho phân trang + tìm kiếm.
   * @type {{page:number,pageSize:number,query:string}}
  ...
  // =========================
  // Utils: text/format helpers
  // =========================

  /**
   * Trả về text an toàn để render UI (fallback "—" nếu rỗng).
   * @param {any} v - Giá trị đầu vào.
   * @returns {string} - Chuỗi hiển thị an toàn.
   * Created By DCHAO - 16/12/2025
   */
  function safeText(v) {
  ...
  /**
   * Render sao dạng FontAwesome (<i class="fa ...">).
   * Yêu cầu: đã load FontAwesome (FA4 dùng fa-star / fa-star-o).
   * @param {any} rate - Đánh giá.
   * @returns {string} - HTML sao hoặc "—".
   * Created By DCHAO - 16/12/2025
   */
  ...
  /**
   * Seed dữ liệu mặc định (window.DEFAULT_CANDIDATES) nếu localStorage chưa có.
   * @returns {Array<object>} - Danh sách ứng viên sau seed.
   * Created By DCHAO - 16/12/2025
   */
  ...
  /**
   * Xóa các candidate theo danh sách id.
   * @param {string[]} ids
   ...
   * @returns {Array<object>} - Danh sách sau lọc.
   * Created By DCHAO - 16/12/2025
   */
   ...
   * @param {Array<any>} list - Danh sách.
   * @param {number} page - Trang hiện tại (1-based).
   * @param {number} pageSize - Số bản ghi/trang.
   * @returns {{total:number,totalPages:number,page:number,pageItems:Array<any>,startIndex:number,endIndex:number}}
   * Created By DCHAO - 16/12/2025
   */
  ...
  /**
   * Render tbody danh sách ứng viên (chỉ render phần đang hiển thị).
   * @param {Array<object>} list - Danh sách ứng viên đã phân trang.
   * Created By DCHAO - 16/12/2025
   */
  ...
  /**
   * Cập nhật footer phân trang (tổng bản ghi, range hiển thị, disable prev/next).
   * @param {{total:number,startIndex:number,endIndex:number,page:number,totalPages:number}} meta - Thông tin phân trang.
   * Created By DCHAO - 16/12/2025
   */
  ...
  /**
   * Render theo state: filter -> paginate -> render table + footer.
   * @param {Array<object>} allCandidates - Danh sách ứng viên nguồn.
   * Created By DCHAO - 16/12/2025
   */
  ...
  /**
   * Load HTML của candidate modal vào #modalMount.
  ...
  /**
   * Gắn event để:
   * - Tự reload danh sách khi modal create/update xong (event candidates:changed)
   * - Mở popup edit khi double click vào 1 dòng ứng viên
   * @param {{getAll:() => Array<object>}} deps - Dependencies để lấy danh sách hiện tại.
   * Created By DCHAO - 16/12/2025
   */
  function bindCandidateListEvents(deps) {
    // 1) Reload list sau khi create/update trong modal
  ...
  /**
   * Bootstrap: load modal + init modal + init danh sách (search/paging).
   * Created By DCHAO - 16/12/2025
   */
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    // nguồn dữ liệu cho list (seed lần đầu, các lần sau lấy localStorage)
    ...
    // NEW: init selection module (đã tách sang candidates.selection.js)
